...
| GET | `/test-runs/{runId}/cases/{caseId}` | Детали одного кейса в контексте run | `internal/api/testRun.go` |
...
| GET | `/test-runs/{runId}/cases/{caseId}` | Получить один case в контексте run (содержимое кейса + результат) |
...
### 5.5 GET `/test-runs/{runId}/cases/{caseId}`
- Назначение: получить один кейс в контексте прогона — для детального просмотра кейса
  со страницы run (deep-link, без загрузки всего прогона).
- Response:
  - Success: `200 OK`, объект:
```json
{
  "run_id": 2,
  "suite_id": 10,
  "run_details": {"name": "Regression run"},
  "case_id": 7,
  "test": {"name": "Login works", "preconditions": "...", "steps": [{"step": 1, "action": "...", "expected_result": "..."}]},
  "status": "passed",
  "comment": "Executed successfully",
  "executed_at": "2026-06-13T10:00:00Z",
  "executed_by": "qa.user"
}
```
  - `test` — текущее содержимое кейса из `test_cases` (JOIN, не снапшот на момент прогона) —
    так же, как в `GET /test-runs/{id}`.
  - Errors:
    - `400 Bad Request` (`invalid_path_param`) — невалидные `runId`/`caseId`;
    - `404 Not Found` — `test_run_not_found` (run не существует) либо
      `test_run_case_not_found` (кейс не входит в этот run).

### 5.6 PATCH `/test-runs/{runId}/cases/{caseId}`
