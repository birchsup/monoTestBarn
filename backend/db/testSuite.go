...
func GetTestSuiteByID(suiteID int) (TestSuite, error) {
	var testSuite TestSuite
...
func UpdateTestSuite(suiteID int, name string, description string) (TestSuite, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) || errors.Is(err, sql.ErrNoRows) {
			return TestSuite{}, ErrTestSuiteNotFound
		}
		return TestSuite{}, err
	}
	return testSuite, nil
...
// DeleteTestSuite deletes a test suite. Linked suite-case rows are removed via
// ON DELETE CASCADE, and runs created from this suite keep their history
// (test_runs.suite_id is set to NULL).
func DeleteTestSuite(suiteID int) error {
	query := `DELETE FROM test_suites WHERE id = $1`
	tag, err := DBPool.Exec(context.Background(), query, suiteID)
	if err != nil {
	...
	if tag.RowsAffected() == 0 {
		return ErrTestSuiteNotFound
	}
	return nil
}

// SuiteExists reports whether a test suite with the given id exists.
func SuiteExists(suiteID int) (bool, error) {
	var exists bool
	err := DBPool.QueryRow(context.Background(),
		`SELECT EXISTS(SELECT 1 FROM test_suites WHERE id = $1)`, suiteID).Scan(&exists)
	return exists, err
}

// LinkTestCaseToSuite links a single case to a suite (idempotent). Returns
// ErrTestCaseNotFound if the case does not exist (FK violation).
func LinkTestCaseToSuite(suiteID int, caseID int) error {
	_, err := DBPool.Exec(context.Background(),
		`INSERT INTO test_suite_cases (suite_id, case_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
		suiteID, caseID)
	if err != nil {
		if isForeignKeyViolation(err) {
...
func RemoveTestCaseFromSuite(suiteID int, caseID int) error {
	...
	tag, err := DBPool.Exec(context.Background(), query, suiteID, caseID)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
		return ErrSuiteCaseLinkNotFound
	}
	return nil
}
